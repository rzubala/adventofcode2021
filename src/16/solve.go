...
func main() {
	...
	stack := make([]SubPackets, 0)
	_, _, sum := parsePacket(hexToBin(packetHex), stack, 0, 0)
	fmt.Println("version sum", sum)
}

func parsePacket(bits []string, stack []SubPackets, state int, sum uint64) (int, int, uint64) {
	ptr := 0
	nextCounter := state
	for ptr+6 <= len(bits) {
		...
		if header == 4 {
			...
			stack = append(stack, SubPackets{id, count})
			nextCounter = 0
			var res int
			if id == 1 {
				res, _, sum = parsePacket(bits[ptr+offset:], stack, 0, sum)
			} else {
				res, _, sum = parsePacket(bits[ptr+offset:], stack, 0, sum)
			}
			if res < 0 {
